...
| `/trust/vio/value` | `VioTrustReport` | `trust_engine_node` | `fusion_node`, `health_monitor_node`, `logger_node` | runtime autonomy | VIO trust derived from VIO estimator quality and freshness |
| `/trust/localization/confidence` | `LocalizationConfidenceReport` | `trust_engine_node` | `fusion_node`, `ew_risk_map_node`, `logger_node` | runtime autonomy | Confidence-aware localization input |
| `/trust/mission_confidence` | `MissionConfidenceReport` | `trust_engine_node` | `mission_continuity_node`, `tactical_summary_node`, `health_monitor_node`, `logger_node` | runtime autonomy | Aggregated mission confidence used by downstream mission continuity and tactical interpretation |
...
Mission decisions are produced only by `mission_continuity_node`. Topics under `/trust/*` must carry trust or confidence values and must not imply control authority.
