...
| `/trust/*` | GNSS trust, source confidence, and trust decisions. | `gnss_trust_node`, `trust_engine_node` | `fusion_node`, `mission_continuity_node`, `ew_risk_map_node`, `tactical_summary_node`, `health_monitor_node`, `logger_node` | Trust outputs must remain independent from `/truth/*`. |
